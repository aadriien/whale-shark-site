...
function scalePointsPerShark(sharkCount: number): number {
    ...
    return Math.max(
        1,
        Math.min(MAX_POINTS_PER_SHARK, Math.floor((10 * POINT_BUDGET) / sharkCount))
    );
}

// Count-only passes to check budget without instantiating point objects
function countAllCoordinates(): number {
    return coordinatesData.reduce((sum, s) => sum + s.coordinates.length, 0);
}
...
export function getAllCoordinates() {
    ...
    const limitResults =
        totalAvailable <= POINT_BUDGET ? Infinity : scalePointsPerShark(coordinatesData.length);
...
export function getGroupCoordinates(allSharkIDs: string[]) {
    ...
    const limitResults =
        totalAvailable <= POINT_BUDGET ? Infinity : scalePointsPerShark(allSharkIDs.length);
...
export function getSharkCoordinates(sharkID: string) {
    ...
    const sharkDict = coordinatesData.find((shark) => shark.whaleSharkID == sharkID);
...
export function getGroupCoordinatesByTimeline(allSharkIDs: string[], month: number, year: number) {
    if (!allSharkIDs) return [];
