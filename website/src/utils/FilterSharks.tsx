import { cleanLifestage, parseSpecificRegion, parseRemarks } from "./DataUtils";
    ...
    return Array.from(
        new Set(
            sharks.flatMap((shark) => {
                const value = (shark as Record<string, unknown>)[fieldSelector] as
                    | string
                    | undefined;
...
export function filterSharks(sharks: WhaleSharkDatasetNormalized, filters: SharkCriteria) {
    return sharks.filter((shark) => {
        // ---------- MEDIA PRESENCE ----------
        if (filters.showOnlyWithMedia) {
            const hasMedia = shark.image && shark.image.trim() !== "" && shark.image !== "Unknown";
        ...
        if (filters.country) {
            ...
            const match = countries.some((c) => c.includes(filters.country.toLowerCase()));
        ...
        if (filters.publishingCountry) {
            ...
            const match = publishingCountries.some((c) =>
                c.includes(filters.publishingCountry.toLowerCase())
            );
            if (!match) return false;
        }
...
export function filterVisionSharks(sharks: WhaleSharkDatasetNormalized, filters: SharkCriteria) {
