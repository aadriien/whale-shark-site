import { parseSpecificRegion, getDate, MONTHS } from "./DataUtils";
...
export function createSummaryDataset(selectedSharkObjects: WhaleSharkDatasetNormalized) {
    ...
    return [
        {
            ...
            Countries:
                countriesArray.slice(0, 3).join(", ") + (countriesArray.length > 3 ? "..." : ""),
            Years:
                yearsArray.length > 0
            ...
            "Year Range":
                yearsArray.length > 0 ? Math.max(...yearsArray) - Math.min(...yearsArray) + 1 : 0,
