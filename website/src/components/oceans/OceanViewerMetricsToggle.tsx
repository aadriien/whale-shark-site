...
const OceanViewerMetricsToggle = ({
    datasetKey,
    onDatasetChange,
}: OceanViewerMetricsToggleProps) => {
    const [expanded, setExpanded] = useState(false);

    const activeConfig = OCEAN_DATASETS[datasetKey as keyof typeof OCEAN_DATASETS];
    const activeGradient = {
        background: `linear-gradient(to bottom, ${activeConfig.gradientStops
            .map((v) => activeConfig.colorScale(v))
            .join(", ")})`,
    };
