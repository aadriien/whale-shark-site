...
function SharkModel({ isAnimating, controlPoints, tetherPosition }: SharkModelProps) {
    ...
    useFrame((state) => {
        ...
        if (isAnimating) {
            ...
            const loopDuration = Math.max(
                10,
                Math.min(30, baseTime + controlPoints.length * timePerPoint)
            );
    ...
    return <primitive ref={sharkRef} object={scene} scale={0.6} />;
}
...
function CurveVisualization({ controlPoints, curveClosed }: CurveVisualizationProps) {
    ...
    const lineRef = useRef<any>(null); // THREE.Line conflicts with SVG <line> in JSX types
    ...
    return (
        <div className="animation-controls">
            ...
            <div className="animation-points-container">
                ...
                {!isAnimating && (
                    <>
                        <button className="add-point-button" onClick={addControlPoint}>
                            Add Point
                        </button>
                        <button
                            ...
                            style={{
                                background:
                                    selectedPointIndex !== null && controlPoints.length > 3
                                        ? "#ff6b6b"
                                        : "#666",
                                cursor:
                                    selectedPointIndex !== null && controlPoints.length > 3
