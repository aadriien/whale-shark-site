...
import { extractContinents, getCountryCode, parseSpecificRegion } from "../../utils/DataUtils";
    ...
    const sharksByContinent = useMemo(() => {
        ...
        console.log(
            ...
            Object.keys(byContinent)
                .map((c) => `${c}: ${byContinent[c as Continent]?.length ?? 0}`)
                .join(", ")
        );
                                ...
                                {isOpen && (
                                    <div className="scrollable-shark-list">
                                        {continentSharks.map((shark) => (
                                            ...
                                            >
                                                {(() => {
                                                    const countryName = parseSpecificRegion(
                                                        (shark.countries ?? "").trim()
                                                    );
