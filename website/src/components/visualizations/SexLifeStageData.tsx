...
const SexLifeStageData = ({ selectedYear, dataset, sharks, title }: SexLifeStageDataProps) => {
    ...
    const chartTitle =
        title ||
        (sharks ? `Lab Sharks Life Stage & Sex` : `Sharks by Life Stage & Sex — ${selectedYear}`);
        ...
        <>
            ...
            sharks ? (
                sharks.length === 0 ? (
                    <ChartPlaceholder
                        type="radialHeatmap"
                        message="Add sharks for life stage + sex"
                    />
                ) : (
                    <p style={{ textAlign: "center" }}>No data available for selected sharks.</p>
                )
            ) : selectedYear ? (
