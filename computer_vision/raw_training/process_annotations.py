...
from transformers import (
    AutoImageProcessor,  # For DINOv2
    AutoModel,  # For MiewID & DINOv2
)

from ..root_constants import OUTPUT_NPZ_FILE
from .raw_training_constants import ANNOTATIONS_PATH, IMAGES_PATH
...
# isinterest == 1 if annotation considered interesting/important for ID tasks
# name == final identity (UUID format) of individual shark associated with annotation
...
def populate_embeddings_database() -> None:
    ...
    with warnings.catch_warnings():
        ...
        miewid_model = AutoModel.from_pretrained(
            MODEL_TAG_MIEWID, trust_remote_code=True
        )
    ...
    for annotation in annotations["annotations"]:
        ...
        with torch.no_grad():
            ...
            # Average pooling over last hidden states
            # (mean over sequence length dimension)
            dinov2_embedding = (
                dino_output.last_hidden_state.mean(dim=1).squeeze().cpu().numpy()
            )
    ...
    print(
        f"Processed {len(image_ids)} image_ids,"
        f" with MIEWID embedding shape: {miewid_embedding.shape}"
    )
