...
from src.copernicus.fetch_chlorophyll import (
...
def aggregate_data_coords(
    dataframe: pd.DataFrame, variables: list[str]
) -> pd.DataFrame:
    ...
    # Compute mean & count of non-NaN values for each variable
    # (how many averaged per bin)
    mean_df = (
        grouped[variables]
        .mean()
        .rename(columns={var: f"mean_{var}" for var in variables})
    )
    count_df = (
        grouped[variables]
        .count()
        .rename(columns={var: f"count_entries_{var}" for var in variables})
    )
...
def convert_xarray_to_df(
    ...
    if missing_vars:
        raise ValueError(
            f"Variables not found in dataset: {missing_vars}. "
...
def export_all_regions_chlorophyll(
    lme_bounds: dict,
    start_year: int = 1997,
    end_year: int = 2024,
    output_folder: str = COPERNICUS_CHLOROPHYLL_DIR,
