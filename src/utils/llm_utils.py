...
IMAGE_COL_STR = "LLM-Gen Image (API)"
...
def prompt_local_LLM(prompt: str, method: Literal["chat", "generate"]) -> str:
    if method not in {"chat", "generate"}:
        raise ValueError("Error, must specify either 'chat' or 'generate' method")
    ...
    # Generate mode for simple output (method == "generate")
        ...
        response = ollama.generate(model=LOCAL_LLM_MODEL, prompt=prompt, stream=False)
...
def name_all_sharks(named_sharks: pd.DataFrame, generated_names: dict) -> pd.DataFrame:
    ...
    named_sharks = move_column_after(
        dataframe=named_sharks, col_to_move=LOCAL_COL_STR, after_col="whaleSharkID"
    )
    named_sharks = move_column_after(
        dataframe=named_sharks, col_to_move=API_COL_STR, after_col=LOCAL_COL_STR
    )
...
def generate_image_huggingface(prompt: str, shark_ID: str) -> None:
    ...
    pipeline = StableDiffusionPipeline.from_pretrained(
        HUGGING_FACE_MODEL, torch_dtype=torch_dtype
    ).to(device)
...
def generate_image_API(prompt: str, shark_ID: str) -> str:
    ...
    try:
        response = requests.get(
            f"{IMAGE_GEN_URL_BASE}/{prompt}?seed={seed}", params=API_IMAGE_PARAMS
        )
        response.raise_for_status()
...
def get_shark_row_image_url(row: pd.Series) -> str:
    ...
    image_prompt = (
        f"Whimsical cartoon of {shark_data[API_COL_STR]}, "
        f"a filter-feeding whale shark. "
        f"This is *not* a scary shark (absolutely *no* pointy teeth)! "
        f"It's a gentle giant with a huge flat mouth "
        f"and soft, speckled skin with white dots. "
...
def get_all_shark_images(
    selected_sharks: pd.DataFrame, generated_images: list
) -> pd.DataFrame:
    selected_sharks[IMAGE_COL_STR] = generated_images.apply(lambda x: x)

    selected_sharks = move_column_after(
        dataframe=selected_sharks, col_to_move=IMAGE_COL_STR, after_col="whaleSharkID"
    )
...
def handle_images() -> None:
    ...
    selected_sharks = story_sharks.loc[
        story_sharks["whaleSharkID"].isin(SELECTED_SHARK_IDS)
    ]

    # Generate LLM images for each row
    relevant_fields_df = selected_sharks[
        SHARK_FIELDS_TO_REVIEW + [LOCAL_COL_STR, API_COL_STR]
    ]
